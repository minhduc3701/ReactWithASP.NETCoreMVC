...
var MarqueeSelectionBase = /** @class */ (function (_super) {
  ...
  MarqueeSelectionBase.prototype.componentDidMount = function () {
    ...
    var hitTarget = this.props.isDraggingConstrainedToRoot
      ? this._root.current
      : this._scrollableSurface;
  ...
  MarqueeSelectionBase.prototype.render = function () {
    ...
    return React.createElement(
      "div",
      __assign({}, rootProps, { className: classNames.root, ref: this._root }),
      children,
      dragRect &&
        React.createElement("div", { className: classNames.dragMask }),
      dragRect &&
        React.createElement(
          "div",
          { className: classNames.box, style: dragRect },
  ...
  MarqueeSelectionBase.prototype._onMouseMove = function (ev) {
    ...
    if (ev.buttons !== undefined && ev.buttons === 0) {
      ...
      if (
        this.state.dragRect ||
        getDistanceBetweenPoints(this._dragOrigin, currentPoint) >
          MIN_DRAG_DISTANCE
      ) {
  ...
  MarqueeSelectionBase.prototype._isPointInRectangle = function (
    rectangle,
    point
  ) {
    return (
  ...
  MarqueeSelectionBase.prototype._isDragStartInSelection = function (ev) {
    ...
    if (
      !this._root.current ||
      (selection && selection.getSelectedCount() === 0)
    ) {
      return false;
    }
    var allElements = this._root.current.querySelectorAll(
    ...
    for (var i = 0; i < allElements.length; i++) {
      ...
      if (selection.isIndexSelected(index)) {
        ...
        if (
          this._isPointInRectangle(itemRect, {
            left: ev.clientX,
            top: ev.clientY,
          })
        ) {
  ...
  MarqueeSelectionBase.prototype._evaluateSelection = function (
    ...
    for (var index1 in this._selectedIndicies) {
      if (this._selectedIndicies.hasOwnProperty(index1)) {
        this._allSelectedIndices[index1] = true;
      }
    }
    ...
    for (var index3 in this._allSelectedIndices) {
      if (
        this._allSelectedIndices[index3] !== previousSelectedIndices[index3]
      ) {
        needToUpdate = true;
        break;
      }
    ...
    if (!needToUpdate) {
      for (var index4 in previousSelectedIndices) {
        if (
          this._allSelectedIndices[index4] !== previousSelectedIndices[index4]
        ) {
          needToUpdate = true;
          break;
        }
    ...
    if (needToUpdate) {
      ...
      selection.setAllSelected(false);
      for (
        var _b = 0, _c = Object.keys(this._allSelectedIndices);
        _b < _c.length;
        _b++
      ) {
        var index5 = _c[_b];
        selection.setIndexSelected(Number(index5), true, false);
      }
