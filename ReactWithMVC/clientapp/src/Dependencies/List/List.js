...
var List = /** @class */ (function (_super) {
  ...
  function List(props) {
    ...
    _this._onRenderSurface = function (props) {
      ...
      return React.createElement(
        "div",
        __assign({ ref: surfaceRef }, divProps),
        pageElements
      );
    };
        ...
        cells.push(
          React.createElement(
            "div",
            {
              ...
              "data-rc-id": rcName ? `cel.${rcName}.${index}` : undefined,
            },
            onRenderCell &&
              onRenderCell(
                item,
    ...
    _this._onAsyncScroll = _this._async.debounce(
    ...
    _this._onAsyncIdle = _this._async.debounce(
      _this._onAsyncIdle,
      IDLE_DEBOUNCE_DELAY,
      {
        leading: false,
    ...
    _this._onAsyncResize = _this._async.debounce(
    ...
    _this._onScrollingDone = _this._async.debounce(
    ...
    for (var _i = 0, pages_1 = pages; _i < pages_1.length; _i++) {
      ...
      if (isPageVisible) {
        if (!measureItem) {
          ...
          for (
            var itemIndex = page.startIndex;
            itemIndex < page.startIndex + page.itemCount;
            itemIndex++
          ) {
            var rowHeight1 = measureItem(itemIndex);
            if (
              page.top + totalRowHeight <= this._scrollTop &&
              this._scrollTop < page.top + totalRowHeight + rowHeight1
            ) {
              return itemIndex;
            } else {
              totalRowHeight += rowHeight1;
            }
          }
        }
      }
  ...
  List.prototype.render = function () {
    ...
    for (var _i = 0, pages_2 = pages; _i < pages_2.length; _i++) {
      var page = pages_2[_i];
      pageElements.push(this._renderPage(page, _i));
    }
  ...
  List.prototype._renderPage = function (page, index) {
    var usePageCache = this.props.usePageCache;
    var rcName = this.props.rcName;
    var cachedPage;
    ...
    var pageElement = onRenderPage(
      {
        ...
        style: pageStyle,
        role: "presentation",
        "data-rc-id": rcName ? `lst.${rcName}` : undefined,
      },
      this._onRenderPage
    );
  ...
  List.prototype._getPageStyle = function (page) {
    ...
    return __assign(
      __assign({}, getPageStyle ? getPageStyle(page) : {}),
      !page.items
        ? {
            height: page.height,
          }
  ...
  List.prototype._notifyPageChanges = function (oldPages, newPages, props) {
    ...
    if (onPageAdded || onPageRemoved) {
      ...
      for (var _a = 0, newPages_1 = newPages; _a < newPages_1.length; _a++) {
        var page1 = newPages_1[_a];
        if (page1.items) {
          if (!renderedIndexes[page1.startIndex]) {
            this._onPageAdded(page1);
          } else {
            delete renderedIndexes[page1.startIndex];
          }
        }
      }
  ...
  List.prototype._buildPages = function (props) {
    ...
    var _loop_1 = function (itemIndex) {
      ...
      var isPageRendered =
        findIndex(this_1.state.pages, function (page) {
          return !!page.items && page.startIndex === itemIndex;
        }) > -1;
      var isPageInAllowedRange =
        !allowedRect ||
        (pageBottom >= allowedRect.top && pageTop <= allowedRect.bottom);
      ...
      var isPageVisible =
        ...
        !shouldVirtualize;
      var isPageFocused =
        focusedIndex >= itemIndex && focusedIndex < itemIndex + itemsPerPage;
      ...
      if (isPageVisible || isPageFocused || isFirstPage) {
        ...
        var newPage = this_1._createPage(
          key,
  ...
  List.prototype._getPageSpecification = function (itemIndex, visibleRect) {
    ...
    if (getPageSpecification) {
      ...
      var itemCount1 = this._getItemCountForPage(itemIndex, visibleRect);
      return {
        itemCount: itemCount1,
        height: this._getPageHeight(itemIndex, visibleRect, itemCount1),
      };
    }
  };
  /**
  ...
  List.prototype._getPageHeight = function (
    ...
    if (this.props.getPageHeight) {
      ...
      return cachedHeight
        ? cachedHeight.height
        : this._estimatedPageHeight || DEFAULT_PAGE_HEIGHT;
    }
  };
  ...
  List.prototype._updateRenderRects = function (props, forceUpdate) {
    ...
    if (
      ...
      surfaceRect = this._surfaceRect = _measureSurfaceRect(
        this._surface.current
      );
...
function _mergeRect(targetRect, newRect) {
  targetRect.top =
    newRect.top < targetRect.top || targetRect.top === -1
      ? newRect.top
      : targetRect.top;
  targetRect.left =
    newRect.left < targetRect.left || targetRect.left === -1
      ? newRect.left
      : targetRect.left;
  targetRect.bottom =
    newRect.bottom > targetRect.bottom || targetRect.bottom === -1
      ...
      : targetRect.bottom;
  targetRect.right =
    newRect.right > targetRect.right || targetRect.right === -1
      ? newRect.right
      : targetRect.right;
