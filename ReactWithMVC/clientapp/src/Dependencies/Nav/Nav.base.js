...
import { composeComponentAs } from "../@uifabric/utilities/componentAs/composeComponentAs";
...
var NavBase = /** @class */ (function (_super) {
  ...
  function NavBase(props) {
    ...
    _this._renderGroup = function (group, groupIndex) {
      ...
      return React.createElement(
        ...
        groupProps.name
          ? onRenderGroupHeader(groupProps, _this._renderGroupHeader)
          : null,
        React.createElement(
          "div",
    ...
    _this._renderGroupHeader = function (group) {
      ...
      var label =
        (isExpanded ? group.collapseAriaLabel : group.expandAriaLabel) ||
      ...
      var onClick = onHeaderClick
        ...
        : undefined;
      let arrNameGroup = group.name.split(" ");
      let NameGroup = arrNameGroup.join("");
      return React.createElement(
        "button",
        {
          ...
          "data-rc-id": rcName ? `${rcName}.${NameGroup}` : undefined,
        },
        React.createElement(Icon, {
          className: classNames.chevronIcon,
          iconName: "ChevronDown",
          rcName: rcName ? `${rcName}.${NameGroup}` : undefined,
        }),
        group.name
      );
    };
  ...
  NavBase.prototype._renderNavLink = function (
    link,
    linkIndex,
    nestingLevel,
    rcName
    ...
    var isSelectedLink = this._isLinkSelected(link, rcName);
    ...
    var rel =
      link.url && link.target && !isRelativeUrl(link.url)
        ? "noopener noreferrer"
        : undefined;
    var LinkAs = this.props.linkAs
      ? composeComponentAs(this.props.linkAs, ActionButton)
      : ActionButton;
    ...
    return React.createElement(
      LinkAs,
      {
        ...
        rcName: rcName ? `${rcName}.${link.key}` : undefined,
  ...
  NavBase.prototype._renderCompositeLink = function (
    link,
    linkIndex,
    nestingLevel,
    rcName,
    ...
    var divProps = __assign(
      {
        // "data-rc-id": rcName ? `${rcName}-${link.key}-link` : undefined
      },
      getNativeProps(link, divProperties, ["onClick"])
    );
    // tslint:disable-next-line:deprecation
    var _a = this.props,
      expandButtonAriaLabel = _a.expandButtonAriaLabel,
    ...
    var classNames = getClassNames(styles, {
      ...
      isSelected: this._isLinkSelected(link, rcName),
      isLink: true,
      isDisabled: link.disabled,
      ...
      link.links && link.links.length > 0
        ? React.createElement(
            "button",
            {
              ...
              "data-rc-id": rcName ? `btn.${rcName}.${link.name}` : undefined,
            },
            React.createElement(Icon, {
              className: classNames.chevronIcon,
              iconName: "ChevronDown",
        ...
        : null,
      this._renderNavLink(link, linkIndex, nestingLevel, rcName)
    );
  };
  ...
  NavBase.prototype._renderLinks = function (links, nestingLevel) {
    ...
    return React.createElement(
      "ul",
      { role: "list", className: classNames.navItems },
      linkElements
    );
  };
  ...
  NavBase.prototype._isLinkSelected = function (link, rcName) {
    ...
    if (this.props.selectedKey !== undefined) {
      ...
      if (window.location.hash) {
        ...
        if (window.location.hash === link.url) {
          return true;
        }
        // Match a rebased url. (e.g. #foo becomes http://hostname/foo)
        _urlResolver.href = window.location.hash.substring(1);
  ...
  NavBase.prototype._toggleCollapsed = function (group) {
    var _a;
    if (group.name) {
      var newGroupCollapsed = __assign(
        __assign({}, this.state.isGroupCollapsed),
        ((_a = {}), (_a[group.name] = this._isGroupExpanded(group)), _a)
      );
