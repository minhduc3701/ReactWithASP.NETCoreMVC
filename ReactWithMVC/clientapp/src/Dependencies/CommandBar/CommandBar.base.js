...
import CommandBarButton from "../Button/CommandBarButton/CustomCommanBarButton";
...
var CommandBarBase = /** @class */ (function (_super) {
  ...
  function CommandBarBase(props) {
    ...
    _this._onRenderItem = function (item) {
      ...
      var commandButtonProps = __assign(
        __assign(
          {
            allowDisabledFocus: true,
            role: "menuitem",
            rcName: rcName ? `${rcName}` : undefined,
          },
          item
        ),
        {
      ...
      if (item.iconOnly && itemText !== undefined) {
        return React.createElement(
          ...
          __assign(
            {
              content: itemText,
              rcName: rcName ? `${rcName}.${item.key}` : undefined,
            },
            item.tooltipHostProps
          ),
          _this._commandButton(item, commandButtonProps)
        );
      }
    ...
    _this._onRenderOverflowButton = function (overflowItems) {
      ...
      var combinedOverflowItems = __spreadArrays(
        overflowButtonProps.menuProps
          ? overflowButtonProps.menuProps.items
          : [],
        overflowItems
      );
      var overflowProps = __assign(
        __assign(
          {
            role: "menuitem",
          },
      ...
      return React.createElement(
        OverflowButtonType,
        __assign(
          { darkMode, rcName: rcName ? `${rcName}.more` : undefined },
          overflowProps
        )
      );
    };
    ...
    _this._onGrowData = function (data) {
      ...
      if (
        movedItem !== undefined &&
        overflowItems.length > minimumOverflowItems
      ) {
        movedItem.renderedInOverflow = false;
        overflowItems = overflowItems.slice(1);
        // if shiftOnReduce, movedItem goes first, otherwise, last.
        primaryItems = shiftOnReduce
          ? __spreadArrays([movedItem], primaryItems)
          : __spreadArrays(primaryItems, [movedItem]);
        var newData = __assign(__assign({}, data), {
          primaryItems: primaryItems,
          overflowItems: overflowItems,
        });
