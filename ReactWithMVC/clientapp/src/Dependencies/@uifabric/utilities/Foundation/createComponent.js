...
import { Customizations, CustomizerContext, assign } from "../index";
...
export function createComponent(view, options) {
  ...
  var Result = function (componentProps) {
    var settings = _getCustomizations(
      options.displayName,
    ...
    if (useState) {
      ...
      componentProps = __assign(
        ...
        // useState(componentProps) // error: React Hook "useState" is called conditionally. React Hooks must be called in the exact same order in every component render
      );
    }
    var theme = componentProps.theme || settings.theme;
    var tokens = _resolveTokens(
      componentProps,
      theme,
      options.tokens,
      settings.tokens,
      componentProps.tokens
    );
    var styles = _resolveStyles(
      componentProps,
      theme,
      tokens,
      options.styles,
  ...
  for (var _a = 0, allTokens_1 = allTokens; _a < allTokens_1.length; _a++) {
    ...
    if (currentTokens) {
      ...
      if (Array.isArray(currentTokens)) {
        currentTokens = _resolveTokens.apply(
          void 0,
          __spreadArrays([props, theme], currentTokens)
        );
      }
...
function _getCustomizations(displayName, context, fields) {
  ...
  return Customizations.getSettings(
