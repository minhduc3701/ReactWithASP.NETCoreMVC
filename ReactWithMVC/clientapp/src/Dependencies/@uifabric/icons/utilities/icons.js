...
import { fontFace, mergeStyles, Stylesheet } from "../../styling";
...
if (stylesheet && stylesheet.onReset) {
  stylesheet.onReset(function () {
    for (var name_1 in _iconSettings) {
      if (
        _iconSettings.hasOwnProperty(name_1) &&
...
export function registerIcons(iconSubset, options) {
  ...
  options = options
    ? __assign(__assign({}, _iconSettings.__options), options)
    ...
    if (_iconSettings[normalizedIconName]) {
      ...
      if (!options.disableWarnings) {
        warn(
          'The icon "' +
            iconName +
            '" tried to unregister but was not registered.'
        );
      }
    }
...
export function registerIconAlias(iconName, mappedToName) {
  _iconSettings.__remapped[normalizeIconName(iconName)] = normalizeIconName(
    mappedToName
  );
}
/**
...
export function setIconOptions(options) {
  _iconSettings.__options = __assign(
    __assign({}, _iconSettings.__options),
    options
  );
}
var _missingIcons = [];
