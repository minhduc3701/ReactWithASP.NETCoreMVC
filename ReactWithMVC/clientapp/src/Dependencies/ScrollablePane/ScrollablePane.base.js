...
var ScrollablePaneBase = /** @class */ (function (_super) {
  ...
  function ScrollablePaneBase(props) {
    ...
    _this._addToStickyContainer = function (
      sticky,
      stickyContainer,
      stickyContentToAdd
    ) {
      ...
      if (!stickyContainer.children.length) {
        ...
        if (!stickyContainer.contains(stickyContentToAdd)) {
          var stickyChildrenElements_1 = [].slice.call(
            stickyContainer.children
          );
          var stickyList_1 = [];
          ...
          for (
            var _i = 0, stickyListSorted_1 = stickyListSorted;
            _i < stickyListSorted_1.length;
            _i++
          ) {
            var stickyListItem = stickyListSorted_1[_i];
            if (
              (stickyListItem.state.distanceFromTop || 0) >=
  ...
  ScrollablePaneBase.prototype.shouldComponentUpdate = function (
    nextProps,
    nextState
  ) {
    return (
  ...
  ScrollablePaneBase.prototype.componentDidUpdate = function (
    prevProps,
    prevState
  ) {
    var initialScrollPosition = this.props.initialScrollPosition;
  ...
  ScrollablePaneBase.prototype.render = function () {
    ...
    let scrPanel = document.getElementById(rcName ? `src.${rcName}` : "src");
    let scrollHeight = 0;
    let clientHeight = 0;
    if (scrPanel) {
      scrollHeight = scrPanel.scrollHeight;
      clientHeight = scrPanel.clientHeight;
    }
    return React.createElement(
      "div",
      __assign({}, getNativeProps(this.props, divProperties), {
        ref: this._root,
      ...
      React.createElement(
        "div",
        {
          id: rcName ? `src.${rcName}` : "src",
          ref: this._contentContainer,
          className: `${classNames.contentContainer}`,
          "data-rc-id": `${rcName ? `scr.${rcName}` : "scr"}`,
          "data-src-visibled": scrollHeight > clientHeight,
