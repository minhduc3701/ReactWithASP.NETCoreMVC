...
import { SELECTION_CHANGE } from "../@uifabric/utilities/selection";
...
var GroupedListSection = /** @class */ (function (_super) {
  ...
  function GroupedListSection(props) {
    ...
    _this._onRenderGroupHeader = function (props) {
      ...
      return React.createElement(GroupHeader, __assign({ rcName }, props));
    };
    ...
    _this._renderSubGroup = function (subGroup, subGroupIndex) {
      var _a = _this.props,
        ...
        group = _a.group,
        compact = _a.compact;
      var nestingDepth = subGroup.level
        ? subGroup.level + 1
        : groupNestingDepth;
      return !subGroup ||
        subGroup.count > 0 ||
  ...
  GroupedListSection.prototype.render = function () {
    ...
    var renderCount =
      group && getGroupItemLimit ? getGroupItemLimit(group) : Infinity;
    var isShowAllVisible =
      group &&
      !group.children &&
      !group.isCollapsed &&
      !group.isShowingAll &&
    ...
    var groupHeaderProps = __assign(
      __assign(__assign({}, headerProps), dividerProps),
      ariaControlsProps
    );
  ...
  GroupedListSection.prototype.forceListUpdate = function () {
    ...
    if (this._list.current) {
      ...
      var subGroup1 = this.refs["subGroup_" + String(0)];
      if (subGroup1) {
        subGroup1.forceListUpdate();
      }
    }
  };
  ...
  GroupedListSection.prototype._getGroupKey = function (group, index) {
    return (
      "group-" +
      (group && group.key ? group.key : String(group.level) + String(index))
    );
  };
  /**
